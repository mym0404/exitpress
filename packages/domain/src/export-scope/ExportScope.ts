...
export const filterPostsByScope = ({
  ...
  return posts.filter((post) => {
    const inCategory = selectedCategoryIds.size === 0 || selectedCategoryIds.has(post.categoryId)
    const inDateRange = (!from || post.publishedAt >= from) && (!to || post.publishedAt <= to)
