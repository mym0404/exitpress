...
const renderMetadata = (metadata: EvidenceMarkdownSection["metadata"]) => {
...
const renderSourceLink = ({ sourceUrl }: { sourceUrl: string }) => `[원문 보기](${sourceUrl})`

const renderImage = (path: string | null, alt: string) =>
  path
    ? `<img src="${escapeHtmlAttribute(path)}" alt="${escapeHtmlAttribute(alt)}" width="300">`
    : null
...
const renderSection = (
  section: EvidenceMarkdownSection,
