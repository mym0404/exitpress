...
- Document paths are stable and path-safe; export rejects selected posts that resolve to the same output path before writing. MDX adapters encode non-ASCII path segments to URL-safe ASCII while keeping visible titles and category metadata unchanged. Frontmatter includes only enabled fields under configured aliases, plus target-required metadata such as Nextra `asIndexPage`.
