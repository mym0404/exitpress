...
- Storybook UI, Markdown preview, bundled capture asset resolution은 `src/ui/features/storybook/*`가 맡는다.
...
- Storybook definition이나 capture asset을 바꾸면 story input HTML, parser inspection path, bundled capture asset, rendered Markdown preview가 같은 source block을 가리키는지 확인한다.
