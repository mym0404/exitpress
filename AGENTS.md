...
## Project Overview
- This repository is a local export tool for public Naver Blog posts.
- It scans posts, parses editor-specific HTML into blocks, renders Markdown, writes assets, and keeps resumable export state.
...
|-- packages/engine/          # Naver fetch, parse, render, export, assets, upload rewrite
...
- Dependency direction is `web -> domain`, `server -> domain, engine`, and `engine -> domain`.
